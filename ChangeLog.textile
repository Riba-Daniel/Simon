...
* Added a --no-autorun program argument which can be used to stop the stories from running on startup.
* Added a --no-load program argument which stops Simon from loading. Really useful for unit testing Simon. Not much else.
* Changed startup so that instead of loading the UI by default, it now starts a HTTP server which waits for commands.
* Added a --ui command argument which loads the UI instead of the HTTP server.
